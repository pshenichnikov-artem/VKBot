        ...
        private async Task EnrichWithAttachments(List<VkMessageItem> messages, List<long> messageIds)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < messages.Count && i < apiResponse.Response.Items.Count; i++)
                {
                    ...
                    messages[i].PeerId = originalPeerId;
                    messages[i].ReplyMessage = apiResponse.Response.Items[i].ReplyMessage;
                    messages[i].Payload = apiResponse.Response.Items[i].Payload;
                }
            }
            catch (Exception ex)
            {
