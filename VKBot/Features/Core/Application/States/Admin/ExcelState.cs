...
public class ExcelState : BaseState
{
    ...
    public override string Description => "📈 Генерация отчёта\nСоздаёт Excel-отчёт по событию с статистикой ответов";
    ...
    public override async Task<StateResult> ExecuteAsync(UserMessage message)
    {
        ...
        {
            return StateResult.Success("⚠️ Ответьте на сообщение с событием, чтобы создать отчёт", StateAction.End);
        }
        ...
        if (eventMsg == null)
        {
            return StateResult.Success("❌ Это не сообщение с событием", StateAction.End);
        }
        ...
        return StateResult.Success($"📈 Отчёт по событию '{eventTitle}' готов! ✨", StateAction.End, attachments: new List<StateAttachment> { attachment });
    }
